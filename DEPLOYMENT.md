...
Dadas las características *stateless* del contenedor Node.js y la necesidad de una base de datos con capacidad vectorial (`pgvector`), se ha optado por una estrategia de despliegue en plataforma **PaaS (Platform as a Service)** utilizando **Railway**, en lugar de aprovisionar infraestructura en bruto en una VPS.
...
- **Entorno:** Se inyectan las variables sensibles (Secretos) de forma nativa en la plataforma, asegurando su encriptación:
  - `MODEL_API_KEY`: Credencial de OpenAI.
  - `JWT_SECRET`: Semilla criptográfica para la firma de sesiones seguras HTTP-Only.
  - `DATABASE_URL`: URI de conexión a la base de datos provisionada.
...
- En lugar de mantener bases de datos complejas aisladas (Pinecone, ChromaDB), usamos el addon nativo de PostgreSQL soportado por Railway. 
